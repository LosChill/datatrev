...
# No significant unexpected correlation found

# Investigate expretise vs know source
...
p <- ggplot(exp_source, aes(x = know_source, y = as.numeric(expertise), color = know_source)) +
...
p_hilite <- ggplot(exp_source, aes(x = know_source, y = as.numeric(expertise), color = know_source)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.6) +
  geom_jitter(position = jitter, alpha = 0.2) +
  scale_y_continuous(breaks = seq(0, 10, by = 1)) +
  labs(
